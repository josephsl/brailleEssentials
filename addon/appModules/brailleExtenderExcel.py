...
from globalPlugins.brailleEssentials.brailleCompat import (
	BrailleBuffer,
	BrailleHandler,
...
class ExcelCellBrailleRegion(NVDAObjectRegion):
	...
	def _routeToCurrentCell(self, braillePos: int) -> None:
		if NVDAObjectHasUsefulText(self.obj):
			try:
...
def _focus_excel_current_at_display_left(handler: BrailleHandler, mainBuffer: BrailleBuffer) -> bool:
	focusRegion = None
...
def refresh_excel_braille_display() -> None:
	...
	handler._doNewObject(
		itertools.chain(
			getFocusContextRegions(regionObj, oldFocusRegions=oldRegions),
			getFocusRegions(regionObj),
		)
	)
